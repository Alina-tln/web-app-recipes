...
class Recipe(Base):
    ...
    author_id = Column(BigInteger, ForeignKey("users.users.id",  ondelete="SET NULL"), nullable=True)
    ...
    def __repr__(self):
        return f"<Recipe(id={self.id}, author_id={self.author_id}, cooking_time={self.cooking_time_in_minutes})>"
...
class UserRecipe(Base):
    ...
    language = relationship("Language", back_populates="user_recipes")
    ingredients = relationship("UserRecipeIngredient", back_populates="user_recipe")

    def __repr__(self):
        return f"<UserRecipe(id={self.id}, user_id={self.user_id}, base_recipe_id={self.base_recipe_id}, title='{self.title}')>"
...
class UserRecipeIngredient(Base):
    ...
    user_recipe_id = Column(BigInteger, ForeignKey("recipes.user_recipes.id", ondelete="CASCADE"), nullable=False,
                            primary_key=True)
    ingredient_id = Column(BigInteger, ForeignKey("recipes.ingredients.id"), nullable=False, primary_key=True)
    quantity = Column(Float, nullable=False)
    unit_id = Column(BigInteger, ForeignKey("recipes.units.id"), nullable=True)
    language_id = Column(BigInteger, ForeignKey("translations.languages.id"), nullable=False)
    ...
    def __repr__(self):
        return f"<UserRecipeIngredient(user_recipe_id={self.user_recipe_id}, ingredient_id={self.ingredient_id}, quantity={self.quantity})>"
...
class Unit(Base):
    ...
    user_recipe_ingredients = relationship("UserRecipeIngredient", back_populates="unit")
